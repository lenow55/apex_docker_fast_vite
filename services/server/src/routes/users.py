...
from schemas.records import UserInSchema, UserOutSchema
from src.env import prod_mode
...
async def login(user: OAuth2PasswordRequestForm = Depends()):
    ...
    response.set_cookie(
        ...
        expires=1800,
        samesite="Lax",
        secure=prod_mode, # установить в true, когда включу https
    )
...
@router.get("/logout")
async def logout():
    content = {"message": "You've successfully logged out. See you late!"}
    response = JSONResponse(content=content)
    response.delete_cookie("Authorization", httponly=True, secure=prod_mode)
